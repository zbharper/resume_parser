...
def get_keyword(field):  # 这个函数的就是用技能表中的技能和解析出来的所有文字来匹配，匹配到了就存入csv中
    def get_conf(conf_name, conf_path='confs/config.yaml'):
        CONFS = yaml.load(open(os.path.join(os.path.dirname(__file__), conf_path), 'rb'), Loader=yaml.FullLoader)
...
class SegmentsMethod(object):
    def __init__(self, config_file='confs/config.yaml'):      
